...
# Patched by Brian Lu, November 2023.
...
# argument is inf; but since they represent indeterminate *finite* quantities,
# multiplying them by zero should give zero.
#
# Just import this module and then call the functions as before, but with
# the extra argument safe_inf=True.
#
...
# Remark: This patch is needed to make ((-inf ⊕ -inf) ⊕ 1) behave the
# same as (-inf ⊕ (-inf ⊕ 1)) or ⊕(-inf, -inf, 1) where ⊕ denotes
# logaddexp or logsumexp. This issue arises in the forward algorithm
# for HMMs or CRFs if the probabilities are represented in the log
# domain and the parameters include structural zeroes.
# 
# I reported this issue to the PyTorch project, and they may fix it soon.
#   https://github.com/pytorch/pytorch/issues/49724

# TODO: Currently, logaddexp assumes that its inputs are scalars.
...
class LogAddExp_safe_inf(torch.autograd.Function):
    ...
    def backward(ctx, grad_output): # type: ignore
        ...
        if input.requires_grad:
            ...
            grad_input, grad_other = None, None
        if grad_output == 0:   # this test assumes that the output is a scalar; else will get an exception
